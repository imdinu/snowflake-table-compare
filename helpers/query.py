...
def exec_from_file(cursor, path_sql, delim=";"):
    sql_queries = Path(path_sql).read_text()
    sql_queries = [q + delim for q in sql_queries.split(delim) if q]
    dfs = [fetch_chunks(cursor.execute(q)) for q in sql_queries]
    return dfs if len(dfs) > 1 else dfs[0]

def exec_from_string(cursor, query_string, delim=";"):
    sql_queries = [q + delim for q in query_string.split(delim) if q]
    dfs = [fetch_chunks(cursor.execute(q)) for q in sql_queries]
    return dfs if len(dfs) > 1 else dfs[0]

def exec_with_replace(cursor, path_sql, replace, delim=";"):
    sql_queries = Path(path_sql).read_text().replace(*replace)
    sql_queries = [q + delim for q in sql_queries.split(delim) if q]
    dfs = [cursor.execute(q).fetch_pandas_all() for q in sql_queries]
    return dfs if len(dfs) > 1 else dfs[0]
